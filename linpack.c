...
static REAL linpack  (long nreps,int arsize, int roll, REAL *totalDgesl, REAL *totalDgefa, long *totalOps);
...
//			arsize=atoi(buf);
	...
	printf("LINPACK benchmark %s, %s precision.\n", argv[0], PREC);
	...
	while ( rounds++ < 5) {
		if ( nrepsR == 1) {
			printf("r ");
			while ( ( runtimeS = linpack( nrepsR, arsize, 1, &tDgeslR, &tDgefaR, &tOpsR)) < 0.1) {
				nrepsR*=2;
			}
			nrepsR = nrepsR * targetRuntimeS / runtimeS;
		}
		printf("r ");
		runtimeS = linpack( nrepsR, arsize, 1, &tDgeslR, &tDgefaR, &tOpsR);
		nrepsR = nrepsR * targetRuntimeS / runtimeS;

		if ( nrepsU == 1) {
			printf("u ");
			while ( ( runtimeS = linpack( nrepsU, arsize, 0, &tDgeslU, &tDgefaU, &tOpsU)) < 0.1) {
				nrepsU *= 2;
			}
			nrepsU = nrepsU * targetRuntimeS / runtimeS;
		}
		printf("u ");
		runtimeS = linpack( nrepsU, arsize, 0, &tDgeslU, &tDgefaU, &tOpsU);
		nrepsU = nrepsU * targetRuntimeS / runtimeS;

//		nreps=1;
//		while (linpack(nreps,arsize, 0, &tDgeslU, &tDgefaU, &tOpsU)<10.)
//			nreps*=2;
//		printf("\n");
		fflush( stdout);
	}
...
static REAL linpack(long nreps, int arsize, int roll,
					REAL *totalDgesl, REAL *totalDgefa,
					long *totalOps) {
	REAL  *a,*b;
...
//	}
	...
	*totalDgefa += tdgefa;
	*totalDgesl += tdgesl;
	*totalOps += 2. * nreps * ops;
	double totalKFlops = *totalOps / ( 1000. * ( *totalDgefa + *totalDgesl));
	printf("%8ld %6.2f %6.2f%% %6.2f%% %6.2f%%  %9.3f %9.3f\n",
		   nreps, totalt, 100.*tdgefa/totalt,
		   100.*tdgesl/totalt, 100.*toverhead/totalt,
		   kflops / 1000.0, totalKFlops / 1000.0);
